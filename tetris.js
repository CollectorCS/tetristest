class Tetris {
    constructor() {
        ...
        this.touchSupported = this.isTouchDevice();
    ...
    setupEventListeners() {
        document.addEventListener('keydown', (e) => {
            // Prevent default scrolling for arrow keys and space when not in input fields
            const isInputField = e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA';
            if (!isInputField && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Space'].includes(e.code)) {
                e.preventDefault();
            }
    ...
    setupTouchControls() {
        if (!this.touchSupported || !this.canvas) return;
        ...
        this.canvas.addEventListener('touchend', (e) => {
            ...
            } else if (absY > absX && absY > swipeThreshold && dy > 0) {
                // Swipe down -> soft drop (a bit faster by moving two steps when possible)
                this.movePiece(0, 1);
    ...
    updateInstructionVisibility() {
        ...
        if (mobileInstructions) {
            mobileInstructions.style.display = this.touchSupported ? 'block' : 'none';
        }
    }
    ...
    updateLevel() {
        ...
        // Each threshold makes the game progressively faster
        const scoreThreshold = Math.floor(this.score / 2000);
        const baseInterval = 1000;
        
        // Progressive speed increase: each threshold decreases interval more aggressively
        // Threshold 0 (0-1999): 1000ms
        // Threshold 1 (2000-3999): 750ms (150ms decrease)
        // Threshold 2 (4000-5999): 550ms (200ms decrease from previous)
        // Threshold 3 (6000-7999): 350ms (200ms decrease from previous)
        // Threshold 4+ (8000+): continues getting faster
        
        if (scoreThreshold === 0) {
            this.dropInterval = baseInterval;
        } else if (scoreThreshold === 1) {
            this.dropInterval = 650; // 2000 score
        } else if (scoreThreshold === 2) {
            this.dropInterval = 450; // 4000 score - faster than 2000
        } else if (scoreThreshold === 3) {
            this.dropInterval = 250; // 6000 score - faster than 4000
        } else {
            // For 8000+ and beyond, decrease by 150ms per additional threshold
            const additionalThresholds = scoreThreshold - 3;
            this.dropInterval = Math.max(50, 450 - (additionalThresholds * 150));
        }
    }
